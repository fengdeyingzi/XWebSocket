...
//		Log.e("isGb",""+start);
	...
	public static String getCoding(byte[] buf){
		int error_gbk = checkByte(buf, "GBK");
		int error_utf = checkByte(buf, "UTF-8");
		if(error_gbk<error_utf){
			return "GBK";
		}
		else{
			return "UTF-8";
	...
	//以指定编码读取字符串，返回转换后byte的错误数
	public static int checkByte(byte[] buf, String coding){
		int error_num = 0;
		try {
			String checkStr = new String(buf,coding);
			byte[] buf_check = checkStr.getBytes(coding);
			int minLen = Math.min(buf.length,buf_check.length);
